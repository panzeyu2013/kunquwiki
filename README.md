...
2. 页面直接使用完整表单，而不是 quick create
3. 提交后调用 `createQuickEntityClient`
4. 请求 `POST /api/editor/quick-create`
5. `ContentRepository.createQuickEntity`
6. Repository 会：
   - 创建 `Entity`
   - 创建专用子表初始记录
   - 使用 `initialData` 调用 `applyStructuredProposal`
   - 写标签、关系等结构化数据
   - 重建搜索索引
7. 前端跳转到新建实体详情页
...
- 简单字段直接用输入框
- 一对多 / 多层嵌套结构，如演出场次、节目单、cast、人物身份履历、人物院团履历，当前通过 JSON 区块完整编辑
...
#### 4. 复杂结构字段当前通过 JSON 编辑

这是为了优先保证“数据库里有的字段都能完整编辑”。后续可以逐步把这些 JSON 区块替换为更友好的可视化子表单。
